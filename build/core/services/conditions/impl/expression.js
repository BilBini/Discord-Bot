...
export default class ExpressionCondition extends Condition {
    ...
    async isMet(condition, context, variables) {
        ...
        if (!input)
            return condition.missingArg("input");
        if (!operator)
            return condition.missingArg("operator");
        if (!output)
            return condition.missingArg("output");
        ...
        if (isNaN(inputNumber))
            return condition.logError("Invalid number in input");
        if (isNaN(outputNumber))
            return condition.logError("Invalid number in output");
        switch (operator) {
            case "==":
