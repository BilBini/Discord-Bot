...
export default class CommandService extends Service {
    ...
    async registerFromDir(commandsDir, plugin = null) {
    ...
    async deployCommands() {
        ...
        try {
            ...
            if (primaryGuildCommands && guild)
                await guild.commands.set(primaryGuildCommands);
            const publicCommands = enabledCommands.filter(cmd => cmd.public === true);
            if (publicCommands)
                await this.manager.client.application?.commands.set(publicCommands);
        }
        catch (error) {
