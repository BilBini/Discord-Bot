...
export default class Utils {
    ...
    static async applyVariables(value, variables, context) {
        if (!value)
            return "";
        if (context?.user)
            variables.push(...this.userVariables(context.user));
        if (context?.channel)
            variables.push(...this.channelVariables(context.channel));
        if (context?.role)
            variables.push(...this.roleVariables(context.role));
        if (context?.content)
            variables.push({ searchFor: "%content%", replaceWith: context.content });
        if (context?.message)
            variables.push({ searchFor: "%message_content%", replaceWith: context.message.content }, { searchFor: "%message_url%", replaceWith: context.message.url });
    ...
    static async logToDiscord(id, message) {
        if (id === 'none')
            return;
